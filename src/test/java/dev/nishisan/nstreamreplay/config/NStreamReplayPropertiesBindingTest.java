...
/**
 * Verifica o binding real ({@link Binder}) de YAML para {@link NStreamReplayProperties} no modelo
 * v2 (sources/sinks/routes), exercitando {@code @DefaultValue}, binding relaxado de enum e
 * {@link Duration}, e o {@code toTopic} opcional das rotas.
 */
class NStreamReplayPropertiesBindingTest {
    ...
    void aplicaDefaultsQuandoCamposEBlocoQueueAusentes() {
        String yaml = """
                nstreamreplay:
                  sources:
                    - { id: s1, bootstrapServers: "host:9092", groupId: "g" }
                  sinks:
                    - { id: d1, bootstrapServers: "host:9092" }
                  routes:
                    - { id: r1, source: s1, fromTopic: t, to: [ { sink: d1 } ] }
                """;
        ...
        assertThat(props.routes().get(0).to().get(0).sink()).isEqualTo("d1");
    }
}
